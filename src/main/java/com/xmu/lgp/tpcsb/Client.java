...
import com.xmu.lgp.tpcsb.measurements.exporter.MeasurementsExporter;
import com.xmu.lgp.tpcsb.measurements.exporter.TextMeasurementsExporter;
...
class ClientThread extends Thread {
     ...
     * 
     * @param tpcs
     *            the TPCS implementation to use
     ...
     */
    public ClientThread(TPCS tpcs, boolean dotransactions, Workload workload, Properties props, int opcount,
    ...
    public void run() {
        ...
        // NOTE: Switching to using nanoTime and parkNanos for time management
        // here such that the measurements
        // and the client thread have the same view on time.

        // spread the thread operations out so they don't all hit the TPCS at
        // the same time
        ...
        try {
            ...
            while (((_opcount == 0) || (_opsdone < _opcount)) && !_workload.isStopRequested()) {
                ...
                if (!_workload.doInvoke(_tpcs, _workloadstate)) {
                    break;
                }
...
public class Client {
     ...
     */
    public static final String TPCS_PROPERTY = "tpcs";

    /**
     * The xml file of Request
     */
     ...
     * The xml file of Request
     */
    public static final String ROUTER_PROPERTY = "router";
    ...
    public static void usageMessage() {
        System.out.println("Usage: java com.xmu.lgp.tpcsb.Client [options]");
        ...
        System.out
                .println("  -tpcs tpcsname: specify the name of the TPCS to use (default: com.xmu.lgp.tpcsb.BasicTPCS) - \n"
                        + "        can also be specified as the \"tpcs\" property using -p");
        System.out.println("  -P propertyfile: load properties from the given file. Multiple files can");
        System.out.println("  -X xmlfile: load properties from the given file. define request");
        System.out.println("  -R router: (e.g. dispatcher | appservice | account )");
        System.out.println("  -p name=value:  specify a property to be passed to the TPCS and workloads;");
        ...
        System.out.println("  " + WORKLOAD_PROPERTY
                + ": the name of the workload class to use (e.g. com.xmu.lgp.tpcsb.workloads.CoreWorkload)");
        System.out.println("");
    ...
    private static void exportMeasurements(Properties props, int opcount, long runtime) throws IOException {
        ...
        try {
            ...
            String exporterStr = props.getProperty(EXPORTER_PROPERTY,
                    "com.xmu.lgp.tpcsb.measurements.exporter.TextMeasurementsExporter");
            try {
    ...
    public static void main(String[] args) {
        ...
        while (args[argindex].startsWith("-")) {
            if (args[argindex].compareTo("-threads") == 0) {
                ...
                status = true;
                argindex++;
            } else if (args[argindex].compareTo("-tpcs") == 0) {
                argindex++;
                if (argindex >= args.length) {
                    usageMessage();
                    System.out.println("Missing argument value for -tpcs.");
                    System.exit(0);
                }
                props.setProperty(TPCS_PROPERTY, args[argindex]);
                argindex++;
            } else if (args[argindex].compareTo("-X") == 0) {
                argindex++;
                if (argindex >= args.length) {
                    usageMessage();
                    System.out.println("Missing argument value for -xml.");
                    System.exit(0);
                }
                props.setProperty(XML_PROPERTY, args[argindex]);
                argindex++;
            } else if (args[argindex].compareTo("-R") == 0) {
                argindex++;
                if (argindex >= args.length) {
                    usageMessage();
                    System.out.println("Missing argument value for -R.");
                    System.exit(0);
                }
                props.setProperty(ROUTER_PROPERTY, args[argindex]);
                argindex++;
                ...
                if (eq < 0) {
                    usageMessage();
                    System.out
                            .println("Argument '-p' expected to be in key=value format (e.g., -p operationcount=99999)");
                    System.exit(0);
                }
        ...
        if (argindex != args.length) {
            ...
            if (argindex < args.length) {
                ...
                System.out
                        .println("An argument specifier without corresponding value was found at the end of the supplied command line arguments.");
            }
            System.exit(0);
        }
        ...
        // get number of threads, target and tpcs
        threadcount = Integer.parseInt(props.getProperty(THREAD_COUNT_PROPERTY, "1"));
        tpcsname = props.getProperty(TPCS_PROPERTY, "com.xmu.lgp.tpcsb.services.BasicTPCS");
        ...
        for (int threadid = 0; threadid < threadcount; threadid++) {
            TPCS tpcs = null;
            try {
                tpcs = TPCSFactory.newTPCS(tpcsname, props);
            } catch (UnknowTPCSException e) {
                System.out.println("Unknown TPCS " + tpcsname);
                System.exit(0);
            }

            int threadopcount = opcount / threadcount;

            // ensure correct number of operations, in case opcount is not a
            // multiple of threadcount
            if (threadid < opcount % threadcount) {
                ++threadopcount;
            }

            ClientThread t = new ClientThread(tpcs, dotransactions, workload, props, threadopcount,
        ...
        if (status) {
            ...
            // at this point we assume all the monitored threads are already
            // gone as per above join loop.
            try {
                statusthread.join();
            } catch (InterruptedException e) {
            }
